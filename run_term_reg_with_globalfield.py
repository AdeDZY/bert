...
class InputExample(object):
    ...
    def __init__(self, guid, text, global_context_text, term_recall_dict=None):
        ...
        self.global_context_text = global_context_text
    ...
    def get_train_examples(self, data_dir):
        ...
        for file_name in train_files:
            ...
            for i, line in enumerate(train_file):
                ...
                global_context_text = tokenization.convert_to_unicode(json_dict["doc"][FLAGS.global_context_field])
                ...
                guid = "train-%s" % docid
                examples.append(
                    InputExample(guid=guid, text=doc_text, global_context_text=global_context_text, term_recall_dict=term_recall_dict)
                )
            train_file.close()
    ...
    def get_test_examples(self, data_dir):
        ...
        for file_name in test_files:
            ...
            for i, line in enumerate(test_file):
                ...
                global_context_text = jdict[FLAGS.global_context_field]
                global_context_text = tokenization.convert_to_unicode(global_context_text)
...
def convert_single_example(ex_index, example, max_seq_length,
    ...
    for i, token in enumerate(global_text_tokens):
        tokens.append(token)
        target_weights.append(0)
        target_mask.append(0)
        segment_ids.append(0)
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
                 target_weights, target_mask, use_one_hot_embeddings):
...
def main(_):
    ...
    processors = {"idcontentsjson": IdContentsJsonDocProcessor,
        ...
        with tf.gfile.GFile(output_predict_file, "w") as writer:
            ...
            for (i, prediction) in enumerate(result):
                ...
                if i >= num_actual_predict_examples:
                    break
                output_line = '\t'.join(['{0} {1:.5f}'.format(t, w) for (t, w) in zip(tokens, logits) if t != "[PAD]"])
