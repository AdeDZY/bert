...
class InputFeatures(object):
    ...
    def __init__(self,
                 ...
                 target_weights,
                 target_mask,
                 target_bigram_weights,
                 target_bigram_span_matrix,
        ...
        self.target_bigram_weights = target_bigram_weights
        self.target_bigram_span_matrix = target_bigram_span_matrix
...
def gen_target_bigram_weights(tokens, bigram_recall_dict):

    def get_next_fulltoken_span(start_index):
        i = start_index
        t = tokens[i]
        while i < len(tokens):
            if tokens[i].startwith('##'):
                t += tokens[i][2:]
                i += 1
        return t, start_index, i-1

    bigram_recall_weights = [0 for _ in range(len(tokens))]
    bigram_recall_span_matrix = [[0 for _ in range(len(tokens))] for _ in range(len(tokens))]
    start_p = 0
    none_stop_tokens = []
    while start_p < 0:
        token, token_s, token_e = get_next_fulltoken_span(start_p)
        if token not in stopwords:
            none_stop_tokens.append((token, token_s, token_e))
        start_p = token_e + 1

    # bigrams
    if len(none_stop_tokens) >= 2:
        for i in range(len(none_stop_tokens)):
            t1, t1s, t1e = none_stop_tokens[i]
            t2, t2s, t2e = none_stop_tokens[i + 1]
            weight = bigram_recall_dict.get(t1 + '_' + t2, 0.0)
            bigram_recall_weights[t1s] = weight
            for j in range(t1s, t2e + 1):
                bigram_recall_span_matrix[t1s][j] = 1

    return bigram_recall_weights, bigram_recall_span_matrix
...
def convert_single_example(ex_index, example, max_seq_length,
    ...
    if isinstance(example, PaddingInputExample):
        return InputFeatures(
            ...
            target_bigram_weights=[0.0] * max_seq_length,
            target_bigram_span_matrix=[[0.0] * max_seq_length] * max_seq_length,
    ...
    text_bigram_target_weights, text_target_bigram_span_matrix = gen_target_bigram_weights(text_tokens, example.bigram_recall_dict)
    assert len(text_target_mask) == len(text_tokens)
    assert len(text_target_weights) == len(text_tokens)
    assert len(text_bigram_target_weights) == len(text_tokens)
    ...
    target_bigram_weights, target_bigram_span_matrix = [], []
    tokens.append("[CLS]")
    segment_ids.append(0)
    target_weights.append(0)
    target_mask.append(0)
    target_bigram_weights.append(0)
    target_bigram_span_matrix.append([0.0] * max_seq_length)
    for i, token in enumerate(text_tokens):
        tokens.append(token)
        target_weights.append(text_target_weights[i])
        target_mask.append(text_target_mask[i])
        target_bigram_weights.append(text_bigram_target_weights[i])
        target_bigram_span_matrix.append([0.0] + text_target_bigram_span_matrix[i] + [0.0] * (max_seq_length - 1 - len(text_target_bigram_span_matrix[i])))
        segment_ids.append(0)
    tokens.append("[SEP]")
    segment_ids.append(0)
    target_weights.append(0)
    ...
    assert len(target_bigram_weights) == max_seq_length
    assert len(target_bigram_span_matrix) == max_seq_length
    ...
    if ex_index < 5:
        ...
        tf.logging.info("target_bigram_weights: %s" % " ".join([str(x) for x in target_bigram_weights]))
        tf.logging.info("target_bigram_span_matrx: ")
        for t in target_bigram_span_matrix:
    ...
    feature = InputFeatures(
        ...
        target_bigram_weights=target_bigram_weights,
        target_bigram_span_matrix=target_bigram_span_matrix,
        is_real_example=True)
    return feature
...
def file_based_convert_examples_to_features(
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        features["target_bigram_weights"] = create_float_feature(feature.target_bigram_weights)
        features["target_bigram_span_matrix"] = create_float_feature(feature.target_bigram_span_matrix)
...
def file_based_input_fn_builder(input_file, seq_length, is_training,
    ...
    name_to_features = {
        ...
        "target_bigram_weights": tf.FixedLenFeature([seq_length], tf.float32),
        "target_bigram_span_matrix": tf.FixedLenFeature([seq_length, seq_length], tf.int64),
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
                 target_weights, target_mask, target_bigram_weights, target_bigram_span_matrix,
...
def create_bigram_model(bert_config, is_training, input_ids, input_mask, segment_ids,
                 target_weights, target_mask, target_bigram_weights, target_bigram_span_matrix,
                 use_one_hot_embeddings):
    """Creates a classification model."""
    model = modeling.BertModel(
    ...
    # mean pooling to get bi-gram embeddings
    target_bigram_span_matrix = tf.to_float(target_bigram_span_matrix)
    bigram_layer = tf.matmul(target_bigram_span_matrix, bert_output_layer)  # sum of bigrams # [batch, seq, hidden_size]
    bigram_layer = bigram_layer/(tf.reduce_sum(target_bigram_span_matrix, axis=-1) + 1)

    bert_output_layer = tf.reshape(bert_output_layer, [-1, hidden_size])  # [batch_size * seq_length, hidden_size]
    tf.logging.info(bert_output_layer.shape)

    bigram_layer = tf.reshape(bigram_layer, [-1, hidden_size])
    tf.logging.info("bigram layer shape: {}".format(bert_output_layer.shape))

    output_u_weights = tf.get_variable(
        "tw_output_u_weights", [1, hidden_size],
        initializer=tf.truncated_normal_initializer(stddev=0.02))

    output_u_bias = tf.get_variable(
        "tw_output_u_bias", [1], initializer=tf.zeros_initializer())

    output_bi_weights = tf.get_variable(
        "tw_output_bi_weights", [1, hidden_size],
        initializer=tf.truncated_normal_initializer(stddev=0.02))

    output_bi_bias = tf.get_variable(
        "tw_output_bi_bias", [1], initializer=tf.zeros_initializer())
    ...
    with tf.variable_scope("loss"):
        ...
        logits_u = tf.matmul(bert_output_layer, output_u_weights, transpose_b=True)  # [batch_size * seq_length, 1]
        logits_u = tf.nn.bias_add(logits_u, output_u_bias)  # [batch_size * seq_length, 1]
        logits_u = tf.reshape(logits_u, [-1, seq_length])
        loss_u = tf.losses.mean_squared_error(
            labels=target_weights,
            predictions=logits_u,
            weights=target_mask,
            reduction=tf.losses.Reduction.SUM_BY_NONZERO_WEIGHTS)
        per_example_loss_u = tf.losses.mean_squared_error(
            labels=target_weights,
            predictions=logits_u,
            weights=target_mask,
            reduction=tf.losses.Reduction.NONE)
        per_example_loss_u = tf.reduce_sum(per_example_loss_u, axis=-1)
        masked_logits_u = logits_u * tf.to_float(target_mask)

        probabilities_u = tf.nn.sigmoid(logits_u) # [batch_size * seq_length, hidden_size]
        masked_probabilities_u = probabilities_u * tf.to_float(target_mask)
        # log_probs = tf.nn.log_softmax(logits, axis=-1)
        #one_hot_labels = tf.one_hot(labels, depth=num_labels, dtype=tf.float32)
        #per_example_loss = -tf.reduce_sum(one_hot_labels * log_probs, axis=-1)
        #loss = tf.reduce_mean(per_example_loss)

        logits_bi = tf.matmul(bigram_layer, output_bi_weights, transpose_b=True)
        logits_bi = tf.nn.bias_add(logits_bi, output_bi_bias)
        logits_bi = tf.reshape(logits_bi, [-1, seq_length])
        loss_bi = tf.losses.mean_squared_error(
            labels=target_weights,
            predictions=logits_bi,
            weights=tf.reduce_sum(target_bigram_span_matrix, axis=-1)/(tf.reduce_sum(target_bigram_span_matrix, axis=-1) + 0.00001),
            reduction=tf.losses.Reduction.SUM_BY_NONZERO_WEIGHTS)

        loss = loss_bi + loss_u

        return loss, per_example_loss_u, masked_logits_u, logits_bi


def model_fn_builder(bert_config, init_checkpoint, learning_rate,
                     num_train_steps, num_warmup_steps, use_tpu,
                     use_one_hot_embeddings, use_bigram):
    ...
    def model_fn(features, mode, params):  # pylint: disable=unused-argument
        ...
        target_bigram_weights = features["target_bigram_weights"]
        target_bigram_span_matrix = features["target_bigram_span_matrix"]
        ...
        (total_loss, per_example_loss, logits_u, logits_bi) = create_bigram_model(
                bert_config, is_training, input_ids, input_mask, segment_ids, target_weights, target_mask,
            target_bigram_weights, target_bigram_span_matrix, use_one_hot_embeddings)
...
def input_fn_builder(features, seq_length, is_training, drop_remainder):
    ...
    all_target_weights = []
    all_target_mask = []
    all_target_bigram_weights = []
    all_target_bigram_span_matrix = []
    ...
    for feature in features:
        ...
        all_target_bigram_weights.append(features.target_bigram_weights)
        all_target_bigram_span_matrix.append(features.target_bigram_span_matrix)
...
def main(_):
    ...
    if FLAGS.do_predict:
        ...
        output_predict_file_u = os.path.join(FLAGS.output_dir, "test_results_u.tsv")
        output_predict_file_bi = os.path.join(FLAGS.output_dir, "test_results_bi.tsv")
        with tf.gfile.GFile(output_predict_file_u, "w") as writer:
            num_written_lines = 0
            tf.logging.info("***** Predict results *****")
            for (i, prediction) in enumerate(result):
                logits_u = prediction["logits_u"]
                tokens = tokenizer.convert_ids_to_tokens(prediction["token_ids"])
                if i >= num_actual_predict_examples:
                    break
                output_line = '\t'.join(['{} {}'.format(t, w) for (t, w) in zip(tokens, logits_u)])
                writer.write(output_line)
                writer.write('\n')
                num_written_lines += 1
        assert num_written_lines == num_actual_predict_examples

        with tf.gfile.GFile(output_predict_file_bi, "w") as writer:
            num_written_lines = 0
            for (i, prediction) in enumerate(result):
                logits_bi = prediction["logits_bi"]
                tokens = tokenizer.convert_ids_to_tokens(prediction["token_ids"])
                spans = prediction["target_bigram_span_matrix"]
                if i >= num_actual_predict_examples:
                    break
                ...
                output_line = '\t'.join(['{} {}'.format(t, w) for (t, w) in zip(bigrams, logits_u)])
                writer.write(output_line)
                writer.write('\n')
                num_written_lines += 1
        assert num_written_lines == num_actual_predict_examples
